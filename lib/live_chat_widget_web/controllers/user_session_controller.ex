defmodule LiveChatWidgetWeb.UserSessionController do
  ...
  def create(conn, %{"_action" => "confirmed"} = params) do
    create(conn, params, "Аккаунт подтверждён.")
  end
  ...
  defp create(conn, %{"user" => %{"token" => token} = user_params}, info) do
    case Identity.login_user_by_magic_link(token) do
      ...
      _ ->
        conn
        |> put_flash(:error, "Ссылка недействительна или устарела.")
  ...
  defp create(conn, %{"user" => user_params}, info) do
    ...
    if user = Identity.get_user_by_email_and_password(email, password) do
      ...
      conn
      |> put_flash(:error, "Неверный email или пароль")
  ...
  def delete(conn, _params) do
    conn
    |> put_flash(:info, "Вы вышли из аккаунта.")
