defmodule LiveChatWidgetWeb.UserAuth do
  use LiveChatWidgetWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias LiveChatWidget.Identity
  alias LiveChatWidget.Identity.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in UserToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_live_chat_widget_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      LiveChatWidgetWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      LiveChatWidgetWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule LiveChatWidgetWeb.PageLive do
        use LiveChatWidgetWeb, :live_view

        on_mount {LiveChatWidgetWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{LiveChatWidgetWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_authenticated, _params, session, socket) do
    ...
    if socket.assigns.current_scope && socket.assigns.current_scope.user do
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "Чтобы получить доступ к этой странице, войдите в аккаунт.")
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Identity.sudo_mode?(socket.assigns.current_scope.user, -10) do
      ...
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "Чтобы получить доступ к этой странице, подтвердите вход ещё раз.")
        |> Phoenix.LiveView.redirect(to: ~p"/users/log-in")
  ...
  def require_authenticated_user(conn, _opts) do
    if conn.assigns.current_scope && conn.assigns.current_scope.user do
      ...
      conn
      |> put_flash(:error, "Чтобы получить доступ к этой странице, войдите в аккаунт.")
