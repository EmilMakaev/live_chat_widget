...
### Деплой и CI/CD (`deploy/`, `.github/workflows/`, `DEPLOY.md`)

Готовые конфиги и пошаговый гайд по разворачиванию на VDS — см. отдельный
раздел [Деплой](#деплой) ниже. `.github/workflows/deploy.yml` — автодеплой на
`git push` в `main` (тесты → SSH на сервер с forced-command ключом → `deploy.sh`),
настройка секретов и защита ветки расписаны в DEPLOY.md, раздел 11.
